local config = require("olmaoster.config.editor.lualine.config").options
...
M.diagnostics = function()
  ...
  local left_sep = hl_str(config.thin_separator_icon.left, "SLSeparator")
  local right_sep = hl_str(config.thin_separator_icon.right, "SLSeparator", "SLSeparator")
  return left_sep .. error_hl .. " " .. warn_hl .. " " .. info_hl .. " ".. hint_hl .. right_sep
end
...
M.mode = {
  ...
  fmt = function(str)
    ...
    if mode[str] then
      ...
      return left_sep .. hl_str(str, "SLNormal") .. right_sep
    end
  end,
}
